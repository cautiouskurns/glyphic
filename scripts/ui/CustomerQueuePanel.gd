...
# Manages the customer queue with cork board grid layout
extends Panel
...
func set_grid_columns(columns: int):
	"""Set number of columns in grid layout"""
	if card_container is GridContainer:
...
func populate_queue():
	...
	for customer in current_queue:
		# Wrap card in Control container to allow rotation in GridContainer
		var card_wrapper = Control.new()
		card_wrapper.custom_minimum_size = Vector2(520, 220)
		card_container.add_child(card_wrapper)

		var card = card_scene.instantiate()
		card_wrapper.add_child(card)

		# Center the card in wrapper (accounting for rotation)
		...
		card_wrapper.modulate.a = 0.0  # Start invisible
		var tween = create_tween()
		tween.tween_property(card_wrapper, "modulate:a", 1.0, 0.3).set_delay(card_index * 0.1)
...
func find_card_by_name(customer_name: String):
	...
	for wrapper in card_container.get_children():
		# Cards are now wrapped in Control containers
...
func pulse_red_border(card):
	...
	# Create cream background style with red border
	var cream_color = Color("#F4E8D8")

	var pulse_style = StyleBoxFlat.new()
	pulse_style.bg_color = cream_color
	pulse_style.corner_radius_top_left = 4
	pulse_style.corner_radius_top_right = 4
	pulse_style.corner_radius_bottom_right = 4
	pulse_style.corner_radius_bottom_left = 4
	...
	pulse_style.shadow_color = Color(0, 0, 0, 0.3)
	pulse_style.shadow_size = 8
	pulse_style.shadow_offset = Vector2(3, 3)

	# Flash to red
	card.add_theme_stylebox_override("panel", pulse_style)
	await get_tree().create_timer(0.25).timeout

	# Flash back to cream with no border
	var normal_style = StyleBoxFlat.new()
	normal_style.bg_color = cream_color
