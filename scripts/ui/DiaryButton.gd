...
# Triggers customer queue panel instead of navigating away from shop
extends Button
...
func _ready():
	...
	# pressed.connect(SceneManager.goto_queue_screen)  # REMOVED
...
func _on_hover_start():
	...
	if not is_panel_open:
		# Brighten button modulate (not just diary cover)
		if hover_tween:
			hover_tween.kill()
